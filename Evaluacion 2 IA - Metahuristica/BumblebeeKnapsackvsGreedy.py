...
def greedy_knapsack():
    """Algoritmo Greedy: selecciona items por mejor ratio valor/peso."""
    """Calcular ratio valor/peso para cada item"""
    items_with_ratio = []
    for i, (name, weight, value) in enumerate(items):
        ratio = value / weight
        items_with_ratio.append((i, name, weight, value, ratio))
    
    """Ordenar por ratio descendente"""
    items_with_ratio.sort(key=lambda x: x[4], reverse=True)
    ...
    for idx, name, weight, value, ratio in items_with_ratio:
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("COMPARACION: BUMBLEBEE vs GREEDY")
    print("="*80 + "\n")
    
    print("Ejecutando BUMBLEBEE ALGORITHM...")
    bumblebee_sol, bumblebee_val, bumblebee_w = bumblebees_knapsack(verbose=False)
    
    print("\nEjecutando GREEDY ALGORITHM...")
    greedy_sol, greedy_val, greedy_w = greedy_knapsack()
    ...
    print("\n[BUMBLEBEE ALGORITHM]")
    print(f"Peso total: {bumblebee_w} kg")
    print(f"Valor total: {bumblebee_val}")
    print("Items seleccionados:")
    for bit, (name, w, v) in zip(bumblebee_sol, items):
        if bit == 1:
            print(f"  - {name} (peso {w} kg, valor {v})")
    
    print("\n[GREEDY ALGORITHM]")
    print(f"Peso total: {greedy_w} kg")
    print(f"Valor total: {greedy_val}")
    print("Items seleccionados:")
    for bit, (name, w, v) in zip(greedy_sol, items):
        if bit == 1:
            print(f"  - {name} (peso {w} kg, valor {v})")
    ...
    print("COMPARATIVA")
    print("="*80)
    diferencia = bumblebee_val - greedy_val
    if diferencia > 0:
        print(f"BUMBLEBEE es MEJOR por {diferencia} puntos de valor")
    elif diferencia < 0:
        print(f"GREEDY es MEJOR por {abs(diferencia)} puntos de valor")
    else:
        print("Ambos algoritmos encontraron la misma solucion")
    print("="*80 + "\n")